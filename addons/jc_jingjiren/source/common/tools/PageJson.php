...
class PageJson
{
    ...
    {
        //剩下数据条数
        $data_residue=intval($this->data_total)-intval($this->page_size)*intval($this->current);
        $data_residue=$data_residue<0?0:$data_residue;
        //剩余页数
        $page_residue=intval($this->page_total)-intval($this->current);
        $page_residue=$page_residue<0?0:$page_residue;
        return [
